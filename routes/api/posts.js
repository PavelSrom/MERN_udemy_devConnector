...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    if (err.kind === 'ObjectId') return res.status(404).json({ msg: 'No post with such ID' })
    return res.status(500).json(err)
  }
})
...
router.delete('/comment/:id/:comment_id', auth, async (req, res) => {
  try {
    ...
    if (comment.user.toString() !== req.user.id) return res.status(401).json({ msg: 'Not allowed to delete comment' })
